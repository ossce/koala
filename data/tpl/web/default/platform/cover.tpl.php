...
<ul class="we7-page-tab">
	<?php  if(!empty($module['isrulefields']) &&!empty($_W['account']) && in_array($_W['account']['type'], array(ACCOUNT_TYPE_OFFCIAL_NORMAL, ACCOUNT_TYPE_OFFCIAL_AUTH))) { ?><li><a href="<?php  echo url('platform/reply', array('module_name' => $module['name'], 'version_id' => intval($_GPC['version_id'])));?>">关键字链接入口 </a></li><?php  } ?>
	<?php  if(!empty($frames['section']['platform_module_common']['menu']['platform_module_cover'])) { ?>
	<li class="active"><a href="<?php  echo url('platform/cover', array('module_name' => $module['name'], 'eid' => $entry_id, 'version_id' => intval($_GPC['version_id'])));?>">封面链接入口</a></li>
	<?php  } ?>
</ul>
...
<?php  if($do == 'module') { ?>
	<div ng-controller="KeywordDisplay" ng-cloak>
		<table class="table we7-table table-hover vertical-middle">
			...
			<tr>
				<td data-url="<?php  echo $menu['url'];?>" data-size="100" class="js-url-qrcode">
				...
				<td>
					<div class="link-group" style="position:relative;">
						<a href="<?php  echo url('platform/cover/post', array('module_name' => $module_name, 'eid' => $menu['eid']));?>" class="color-default">编辑</a>
						<a href="javascript:;" data-url="<?php  echo $menu['url'];?>" style="margin-right:0px;" class="color-default js-clip">复制链接</a>
					</div>
				</td>
			</tr>
			<?php  } } ?>
