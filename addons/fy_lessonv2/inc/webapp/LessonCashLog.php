...
$title = $teacher_page['cashLog'] ? $teacher_page['cashLog'] : '提现记录';
...
$member = pdo_fetch("SELECT a.nopay_lesson,a.pay_lesson,b.teacher,b.addtime FROM " .tablename($this->table_member). " a LEFT JOIN " .tablename($this->table_teacher). " b ON a.uid=b.uid WHERE a.uniacid=:uniacid AND a.uid=:uid", array(':uniacid'=>$uniacid,':uid'=>$uid));

$mobile_site_root = $setting_pc['mobile_site_root'] ? $setting_pc['mobile_site_root'] : $_W['siteroot'];
$wap_cash_url = $mobile_site_root."app/index.php?i={$uniacid}&c=entry&op=display&do=lessoncash&m=fy_lessonv2";
...
if($op=='display'){
	...
	if($member['nopay_lesson'] < $comsetting['cash_lower_teacher']){
		$data = array(
			'code' => -1,
			'message' => "当前提现最低额度为{$comsetting['cash_lower_teacher']}元，您的可提现额度不够",
		);
		$this->resultJson($data);
	}
...
include $this->template("../webapp/{$template}/lessonCashLog");
