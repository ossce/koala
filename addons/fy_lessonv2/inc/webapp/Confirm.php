...
if (!empty($nopay_order)) {
	message("您还有该课程未付款的订单", $_W['siteroot']."{$uniacid}/mylesson.html?status=nopay", "warning");
}
...
if(!empty($pay_order) && $pay_order['lesson_type']!=1){
	message("您已购买该课程，无需重复购买", $_W['siteroot']."{$uniacid}/lesson.html?id={$id}", "error");
}
...
if($lesson['support_coupon']==1){
	...
	foreach($coupon_list as $k=>$v){
		$v['amount'] = floor($v['amount'])==$v['amount'] ? intval($v['amount']) : round($v['amount'], 1);
		if($v['use_type']==1){
...
include $this->template("../webapp/{$template}/confirm");
