...
if (!empty($nopay_order)) {
	$jsonData = array(
		'code' => -1,
		'message' => '您还有该课程未付款的订单，请到我的课程里查看',
	);
...
if ($setting['mustinfo']) {
	...
	$jumpurl = $_W['siteroot']."{$uniacid}/memberInfo.html?lessonid={$id}&spec_id={$spec_id}&type=buylesson";
...
if($coupon_id>0 && $lesson['support_coupon']){
	...
	if($price < $coupon['conditions']){
		$jsonData = array(
			'code' => -1,
			'message' => "支付金额未满".$coupon['conditions']."元，无法使用该优惠券",
		);
		$this->resultJson($jsonData);
	}
	...
	if($coupon['use_type']==1){
		if($coupon['category_id'] && $lesson['pid']!=$coupon['category_id']){
			$jsonData = array(
				'code' => -1,
				'message' => "优惠券不支持该课程分类使用",
			);
			$this->resultJson($jsonData);
		}
	}elseif($coupon['use_type']==2){
...
if(intval($_GPC['deduct_integral'])>0 && $price>0){
	...
	if($market['deduct_switch']==1 && $market['deduct_money']>0 && $lesson['deduct_integral']>0 && $member['credit1']>0){
		...
		if($member['credit1'] < $deduct_integral){
			$jsonData = array(
				'code' => -1,
				'message' => "用户积分余额不足".$deduct_integral,
			);
			$this->resultJson($jsonData);
		}
		if($deduct_integral > $lesson['deduct_integral']){
			$jsonData = array(
				'code' => -1,
				'message' => "当前课程最多可使用".$lesson['deduct_integral']."积分",
			);
			$this->resultJson($jsonData);
		}
...
if ($comsetting['is_sale'] == 1 && $spec['spec_price'] > 0) {
    ...
    if ($orderdata['member1'] > 0 && $price>0) {
    ...
    if ($orderdata['member2'] > 0 && $price>0 && in_array($comsetting['level'], array('2', '3'))) {
    ...
    if ($orderdata['member3'] > 0 && $price>0 && $comsetting['level'] == 3) {
...
if ($orderid) {
	...
	$jsonData = array(
		'code' => 0,
		'message' => "提交订单成功",
		'gourl' => $_W['siteroot']."{$uniacid}/payment.html?orderid={$orderid}&ordertype=buylesson"
	);
	$this->resultJson($jsonData);
