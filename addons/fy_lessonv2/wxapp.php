...
/***************************** 接口数据(实际使用) *********************************/
	...
	public function doPageFollowQrcode(){
		...
		/* 关注公众号小程序码 */
		$wxapp_follow = $dirpath."wxapp_follow.png";
		if(!file_exists($wxapp_follow) || filectime($wxapp_follow) > time()+86400){
			$response1 = $account_api->getCodeUnlimit('1047', 'fy_lessonv2/pages/follow/index', 450, array(
				'auto_color' => true,
				'line_color' => array(
					'r' => '#ABABAB',
					'g' => '#ABABAC',
			...
			file_put_contents($wxapp_follow, $response1);
		}

		/* 跳转课堂题库小程序码 */
		$wxapp_exam = $dirpath."wxapp_exam.png";
		if(!file_exists($wxapp_exam) || filectime($wxapp_exam) > time()+86400){
			$response2 = $account_api->getCodeUnlimit('1047', 'fy_lessonv2/pages/exam/index', 450, array(
				'auto_color' => true,
				'line_color' => array(
					'r' => '#ABABAB',
					'g' => '#ABABAC',
			...
			file_put_contents($wxapp_exam, $response2);
		}

		return $this->result(0, '', $_W['uniacid']);
	}

	/* 课堂题库页面(需购买安装课堂题库插件才可以用) */
	public function doPageExam() {
		global $_W;
		
		$url = $_W['siteroot'].'app/index.php?i='.$_W['uniacid'].'&c=entry&type=2&do=examlist&m=fy_lessonv2_plugin_exam';
		return $this->result(0, '', $url);
	}
	...
	public function doPayGetopenid() {
		...
		/* 直播课程打赏订单 */
		if($order_type=='T'){
			$awardorder = pdo_get($this->table_live_award, array('ordersn'=>$ordersn));
			$data['real_url'] = $_W['siteroot'].'app/'.str_replace("./", "", $this->createMobileUrl('lesson', array('id'=>$awardorder['lessonid'],'play'=>1)));
		}
		/* 课堂商城订单 */
		if($order_type=='G'){
			$shoporder = pdo_get($this->table_shop_order, array('ordersn'=>$ordersn));
			$data['real_url'] = $_W['siteroot']."app/index.php?i={$shoporder['uniacid']}&c=entry&do=shoporder&m=fy_lessonv2_plugin_shop";
		}
	...
	public function doPagePay() {
		...
		if($order_type=='A'){
			$awardorder = pdo_get($this->table_live_award, array('ordersn'=>$ordersn));
			$fee = $awardorder['price'];
			$real_url = $_W['siteroot'].'app/'.str_replace("./", "", $this->createMobileUrl('lesson', array('id'=>$awardorder['lessonid'],'play'=>1)));
		}
		/* 购买商城订单 */
		if($order_type=='G'){
			$shoporder = pdo_get($this->table_shop_order, array('ordersn'=>$ordersn));
			$fee = $shoporder['total_amount'];
			$real_url = $_W['siteroot']."app/index.php?i={$shoporder['uniacid']}&c=entry&do=shoporder&m=fy_lessonv2_plugin_shop";
		}
		...
		/* 订单类型 1.vip订单 2.课程订单 3.讲师订单 4.直播打赏订单 5.课堂商城订单 */
		if(!empty($viporder)){
		...
		}elseif(!empty($awardorder)){
			$pay_params['order_type'] = 4;
		}elseif(!empty($shoporder)){
			$pay_params['order_type'] = 5;
		}
		$pay_params['real_url'] = $real_url;
...
/***************************** 接口数据(审核使用) *********************************/
	...
	public function doPageBanner() {
		...
		$banner = array(
			'0' => array(
				'img' => $_W['siteroot'].'addons/fy_lessonv2/static/public/DemoPic/m_banner3.jpg',
			),
			'1' => array(
				'img' => $_W['siteroot'].'addons/fy_lessonv2/static/public/DemoPic/m_banner2.jpg',
			),
		);
	...
	private function _auditWxapp(){
		global $_W;

		$modules = cache_load('fy_lesson_'.$_W['uniacid'].'_modules_wxapp');
		if(empty($modules)){
			$modules = pdo_get('modules',array('name'=>base64_decode('ZnlfbGVzc29udjJfcGx1Z2luX3d4YXBw')));
			if(empty($modules)){
				exit();
			}else{
				cache_write('fy_lesson_'.$_W['uniacid'].'_modules_wxapp', $modules);
			}
		}
	}
