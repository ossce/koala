...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    if (res.url) {
        url = res.url;
        str = "<div><strong>Link:</strong><a> " + res.url + "</a></div>" +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    } else {
        var domain = up.getOption('domain');
        url = domain + encodeURI(res.key);
        var link = domain + res.key;
        str = "<div><strong>Link:</strong><a> " + link + "</a></div>" +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    }
    ...
    var showImg = $('<img src="../addons/fy_lessonv2/static/mobile/default/images/loading.gif" style="width:80px;height:80px;"/>');

    progressNameTd.append(Wrapper);

    if (!isImg) {
        showImg.attr('src', '../addons/fy_lessonv2/static/mobile/default/images/media.jpg');
        ...
        $(img).on('load', function() {
            ...
            showImg.attr('src', '../addons/fy_lessonv2/static/mobile/default/images/pictrue.jpg');
            Wrapper.addClass('default');
        });
    }
};
