require(['jquery', 'jquery.ui', 'underscore', 'ueditor', 'select2', 'fileUploader'], function($, jQueryUI, UnderScore, UEditor, Select2, FileUploader){
    window.Diy = {
        Designer: {
            init: function () {
                ...
                this.Modal = {
                    delete: $('.modal-delete'),
                    selectItem: $('.modal-select-lesson'),
                    itemContent: $('.modal-select-lesson .item-content'),
                    searchButton: $('.modal-select-lesson .search-button'),
        ...
        Editor: {
            ...
            UI: {
                ...
                Common: {
                    ...
                    Item: {
                        init: function () {
                            var obj = $('.ep-select-lesson', Diy.Designer.Editor.content);
                            if (!obj.length) {
                                return;
                            }
							if(fylesson.development){
                            ...
                            obj.each(function () {
                                $(this).unbind('click').click(function () {
                                    ...
                                    var searchFunc = function () {
                                        ...
                                        var data = 'keyword=' + keyword.val() + '&pid=' + pid + '&cid=' + cid + '&lessonType=' + lessonType + '&lessonStatus=' + lessonStatus;
                            ...
                            $.ajax({
                                ...
                                success: function (resp) {
                                    ...
                                    tpl += '<% _.each(data, function(item) { %>'+
                                        '<div class="item" data-item-id="<%= item.id %>" data-item-bookname="<%= item.bookname %>" data-item-images="<%= item.images %>" style="background: #eee url(<%= item.images %>); background-size: cover;">'+
                                    ...
                                    $('.item', Diy.Designer.Modal.itemContent).unbind('click').click(function () {
                                        ...
                                        if (typeof cb == 'function') {
                                            var itemData = {
                                                lessonid: $(this).data('item-id'),
                                                bookname: $(this).data('item-bookname'),
                                                cover: $(this).data('item-images'),
                                            };
                                            cb(itemData);
                                        }
                                    });
                        ...
                        select: function (data) {
                            ...
                            $('[data-key=lessonid]', obj).val(data.lessonid);
                            $('[data-key=bookname]', obj).val(data.bookname);
                        ...
                        paging: function (url, page, ele) {
							...
							data += '&pid=' + $('#pid').val();
							data += '&cid=' + $('#cid').val();
							data += '&lessonType=' + $('#lessonType').val();
							data += '&lessonStatus=' + $('#lessonStatus').val();
                            data += '&page=' + page;
                            Diy.Editor.UI.Common.Item.search({
                                url: Diy.Designer.Modal.selectItem.data('url'),
                                data: data,
				...
				teacher: {
                    init: function () {
						if(fylesson.development){
							fylesson.log('Editor UI teacher init');
						}
				...
				reclesson: {
                    init: function () {
						if(fylesson.development){
							fylesson.log('Editor UI reclesson init');
						}
                ...
                if (!obj.length) {
                    ...
                    if (name == 'page') {
                        data = {
                            ...
                            page_desc: $('input[name=page_desc]').val()
                        };
                    } else {
                        if (typeof Diy.Editor.data[guid] == 'undefined') {
                            data = {
    ...
    $('.btnSave').click(function () {
		...
		var page_desc = $('input[name=page_desc]').val();
        ...
        data += '&page_title=' + page_title + '&page_desc=' + page_desc + '&page_type=' + page_type + '&cover=' + cover;
